...
- `ip` - display or configure network interface parameters, including IP addresses

- `ifconfig` - display or configure network interface parameters, largely replaced by ip

- `ping` - sends a data request (ICMP request) to a specific device, most often a network host

  - If the connection is successful, ping returns information about the connection status.

- `traceroute` - returns a list of devices (the packet route) that a message traveled through

- `netstat` - returns network status information, including current connections, ports in use, and active processes

  - `sudo netstat -tulp` - Check open ports

- `tcpdump` - analyze system network traffic
